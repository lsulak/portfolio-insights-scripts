...
class SECFetcher(ISECFetcher):
    ...
    async def fetch_latest_filings(
    ...
    ) -> Optional[List[LocalSECDocument]]:
        """
        ...
        for curr_file in downloaded_files:
            ...
            logger.debug(
                f"[SEC] Downloaded file for {ticker}, {form_type}, submission year: {four_digit_submission_year}, order: {submission_order_for_the_year}"
            )
...
class GeminiFileManager(IGeminiFileManager):
    ...
    async def upload_for_inference(self, document: LocalSECDocument) -> GeminiHostedFile:
        ...
        return GeminiHostedFile(file_uri=uploaded_file.uri, file_name=uploaded_file.name, mime_type=document.mime_type)
...
class ExtractorAgent(IExtractorAgent):
    ...
    async def generate_structured_dossier(self, ai_file: GeminiHostedFile, system_prompt: str) -> str:
        ...
        """
        logger.info(f"[AGENT] Extracting financials from {ai_file.file_name}...")
