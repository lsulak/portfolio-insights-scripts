"""Edgar extraction pipeline - Orchestrates the complete extraction workflow."""
...
from helios.utils.commons import GeminiFileManager, GeminiExtractorAgent
...
class EdgarExtractionPipeline:
    """Manages the complete Edgar extraction and summarization workflow."""
    ...
    def __init__(self, client: genai.Client, output_base_dir: str, ticker: str, force_resummarize: bool = False):
        ...
        self._gemini_extractor: Optional[GeminiExtractorAgent] = None
    ...
    def _setup_directories(self) -> None:
        ...
        for form_type in EDGAR_FORM_TO_AGENT_SPEC.keys():
    ...
    def _initialize_services(self) -> None:
        ...
        self._edgar_fetcher = EdgarFetcher(
            company_name=EDGAR.company_name, email_address=EDGAR.email, download_dir=self.dir_raw
        )
        ...
        self._gemini_extractor = GeminiExtractorAgent(self.client, model_name=GEMINI.extractor_model)
    ...
    async def _process_document(
        ...
        """Process a single Edgar document through the extraction pipeline.
        ...
        try:
            async with self._llm_semaphore:
                ...
                ai_file = await self._gemini_file_manager.upload_for_inference(
                    doc.file_path_ai_ready, doc.mime_type
                )
    ...
    async def _fetch_and_queue_documents(self) -> list:
        """Fetch documents from Edgar and queue them for processing.
        ...
        for form_type, agent_specs in EDGAR_FORM_TO_AGENT_SPEC.items():
