...
AGENT_SPECS_FILEPATH = AGENT_SPECS_DIR / "sector_analysis.md"

logger = logging.getLogger(__name__)


class SectorAnalyser:
    """Orchestrates the extraction of sector analysis using Gemini Deep Research Agent."""
    ...
    def construct_report_filename(output_base_dir: str, ticker: str) -> str:
        """Construct the output filename for the sector analysis report."""
        curr_quarter = ((int(time.strftime("%m")) - 1) // 3) + 1
        curr_year = time.strftime("%Y")

        output_report_basedir = os.path.join(output_base_dir, ticker, "sector_analysis")
    ...
    def run(self) -> None:
        """Extract and summarize sector analysis using Gemini Deep Research Agent."""
        ...
        if not self.force_resummarize and os.path.exists(output_filename):
            logger.info(f"Sector analysis report already exists at {output_filename}. Skipping extraction.")
            return

        agent_spec = self.construct_agent_spec(self.ticker)
        produced_insight = deep_research_execution_sync(
            self.client, model=GEMINI.sector_analysis_model, agent_spec=agent_spec
        )
        ...
        logger.info(f"Sector Analysis summary written to file '{output_filename}'")
