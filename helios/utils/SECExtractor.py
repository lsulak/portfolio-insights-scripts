...
class SECExtractor:
    ...
    PATTERN_UUENCODE = re.compile(r"begin\s+[0-7]{3}\s+[^\r\n]+\r?\n.*?\r?\nend[ \t]*\r?\n", re.DOTALL | re.IGNORECASE)
    PATTERN_SGML_BIN = re.compile(r"<(PDF|GRAPHIC|ZIP|EXCEL)[^>]*>.*?</\1>", re.DOTALL | re.IGNORECASE)
    ...
    PATTERN_INLINE_TAGS = re.compile(r"</?(div|span|font|a|b|i|u|strong|em|center)[^>]*>", re.IGNORECASE)
    ...
    def clean_and_minify(self, report: LocalSECDocument, max_chars: int = 950000) -> str:
        """
    ...
    def _save_cleaned_file(self, report: LocalSECDocument, content: str) -> str:
        ...
        output_path = os.path.join(
            self._target_dir, report.form_type, f"{report.submission_year}_{report.submission_order_for_the_year}.txt"
        )
