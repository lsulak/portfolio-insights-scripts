...
You MUST format your output strictly using the following Markdown headers and logic. Do not write data in paragraphs; use clear Markdown tables for all financial presentations.

## A. Company Overview
* **Company Overview:** Using the `BE` payload, present the company from its beginnings to the present. Explain the exact physical/digital value chain.
* **Last 10 Years:** Using the `BE` payload, identify the 3 to 7 most critical 'structural breaks' or pivots in the last decade. Focus on these nodes of evolution rather than a linear summary.

## B. Financial Strength & Valuation (Damodaran)
* **The Balance Sheet Test:** Using the `QBC`, present the debt, cash, and interest coverage. Is the company fragile?
* **Valuation:** Extract and present the exact results from the `VE` payload. Show the Intrinsic Valuation (explicitly noting the 15% required rate of return and the Growth Dependency Ratio), the Relative Valuation (vs. historical medians and peers), and the Reverse DCF expectations. *Constraint: Do NOT recalculate the DCF; map the VE's output.*
